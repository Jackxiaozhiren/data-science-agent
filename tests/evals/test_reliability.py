...
def _ev(task_id: str, cat: str, ok: bool, tool_calls: list[dict] | None = None) -> EvaluationResult:
    tc = tool_calls or (
        [{"tool": "run_sql", "status": "ok"}] if ok else [{"tool": "run_sql", "status": "error"}]
    )
    return EvaluationResult(
        ...
        metrics=TaskMetrics(
            task_success=ok,
            code_execution_success=ok,
            evidence_coverage=ok,
            unsupported_claim=not ok,
            latency_ms=100,
        ),
...
def _task(**kw) -> BenchmarkTask:
    base = dict(
        ...
        question="q",
        expected_analysis="e",
        ground_truth=GroundTruth(expected_tool="run_sql"),
        criteria=EvaluationCriteria(task_success=True),
        difficulty="easy",
    )
    base.update(kw)
...
def test_reliability_sections() -> None:
    results = [
        _ev("t1", "SQL", True),
        _ev("t2", "EDA", True, [{"tool": "profile_dataset", "status": "ok"}]),
    ]
    raw = [
        {
            "run_result": {
                "tool_calls": [{"tool": "run_sql", "status": "ok"}],
                "validation_results": [{"check": "unsupported_claim", "passed": False}],
                "retry_count": 1,
                "insights": [{"finding": "ok"}],
            }
        }
...
def test_tool_selection_and_loop() -> None:
    results = [
        _ev(
            ...
            "SQL",
            True,
            [{"tool": "run_sql", "status": "ok"}, {"tool": "run_sql", "status": "ok"}],
        )
    ]
    raw = [
        {
...
def test_statistical_correctness_from_evaluator_v2() -> None:
    ...
    rr = {
        "tool_calls": [
            {"tool": "correlation_analysis", "status": "ok", "output": {"r": 0.5, "p_value": 0.01}}
        ]
    }
    ...
    ev = _ev(
        ...
        "Statistics",
        True,
        [{"tool": "correlation_analysis", "status": "ok", "output": {"r": 0.5}}],
    )
