...
def test_matrix_no_fabrication() -> None:
    ...
    assert by["open_api"].available is False or os.getenv("OPENAI_API_KEY") or os.getenv("ANTHROPIC_API_KEY")
    assert by["frontier"].available is False or os.getenv("OPENAI_API_KEY") or os.getenv("ANTHROPIC_API_KEY")
    # Local is available
...
def test_matrix_with_real_agg_and_frontier() -> None:
    agg = {"task_success_rate": 1.0, "statistical_accuracy": 1.0, "evidence_coverage": 1.0, "mean_latency_ms": 42, "n": 10}
    m = build_cross_model_matrix({"local_small": agg, "local_medium": agg}, token_estimates={"local_small": (100, 100), "local_medium": (120, 120)})
    by = {r.model_class: r for r in m.records}
