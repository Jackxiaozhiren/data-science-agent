...
def test_dimensions_present() -> None:
    ...
    res = evaluate_statistical(
        t,
        {"tool_calls": [{"tool": "correlation_analysis", "status": "ok", "output": {"r": 0.5, "p_value": 0.01, "ci_low": 0.1, "ci_high": 0.7, "effect_size": 0.5}}]},
    )
    for dim in (
        "method_selection",
...
def test_s10_uncertainty_omission() -> None:
    ...
    res = evaluate_statistical(
        t,
        {"tool_calls": [{"tool": "correlation_analysis", "status": "ok", "output": {"r": 0.2}}], "insights": []},
    )
...
def test_attach_via_metrics() -> None:
    ...
    t = _task()
    ev = evaluate_task(t, {"tool_calls": [{"tool": "correlation_analysis", "status": "ok", "output": {"r": 0.4, "p_value": 0.02}}], "report_markdown": "ok"})
    from dsa_evaluation.statistical_eval import evaluate_statistical as eval2

    stat = eval2(t, {"tool_calls": [{"tool": "correlation_analysis", "status": "ok", "output": {"r": 0.4, "p_value": 0.02}}]})
