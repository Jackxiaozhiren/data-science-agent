...
async def test_run_sql_basic() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        tool = get("run_sql")
        r = await tool.run(
            {"sql": "SELECT COUNT(*) as n FROM dataset", "dataset_path": str(p), "max_rows": 100}
        )
...
async def test_run_python_basic() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        r = await tool.run(
            {
                "code": "print(df.shape)\nresult = df.select(pl.col('a').mean()).item()",
                "dataset_path": str(p),
            }
        )
...
async def test_hypothesis_welch() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        r = await tool.run(
            {"dataset_path": str(p), "test": "welch_t_test", "group_col": "group", "value_col": "a"}
        )
        assert r.status == "ok", r.error
        assert r.output is not None
...
async def test_hypothesis_mannwhitney() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        tool = get("hypothesis_test")
        r = await tool.run(
            {"dataset_path": str(p), "test": "mann_whitney", "group_col": "group", "value_col": "b"}
        )
        assert r.status == "ok", r.error
...
async def test_regression_linear() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        r = await tool.run(
            {"dataset_path": str(p), "target": "a", "features": ["b"], "model": "linear"}
        )
        assert r.status == "ok", r.error
        assert r.output is not None
