...
`uv run dsa demo` — `Demo Dataset → Analysis → Evidence → Report` (`demo/runs/demo`); `uv run dsa external-validation` — installation metrics (`Cold/First Launch/Demo/Benchmark`). Linux/macOS tested, Windows explicitly not tested (§41).
