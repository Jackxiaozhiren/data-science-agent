...
- Web-surface audit only: repo README, arXiv abstract, HF dataset page (gated). The evaluator's exact input schema is verified at implementation time from `experiments/evaluate.py` at the pinned commit — if conversion proves impossible without modifying the evaluator, §16 forces reporting `PARTIALLY SUPPORTED` with the blocker documented, not a custom easier evaluator.
- No DataSciBench content has been downloaded or committed to this repository.
