...
The GitHub Release was already published and marked immutable when the v4.3.0 Publish workflow reached its binary-asset attachment step. GitHub correctly rejected post-publish asset uploads because immutable releases only permit assets to be added before publication.
...
The post-release workflow was hardened on `main` so future releases preserve built wheel/sdist files as a retained Actions artifact before publication, pin the publishing toolchain more tightly, and treat attachment to a mutable GitHub Release as best-effort rather than a reason to mark an otherwise successful PyPI publication as failed. The release announcement can therefore continue even when an immutable Release rejects duplicate binary attachments.
...
| Ruff / formatting | ✅ PASS | Final tagged main CI |
| mypy | ✅ PASS | Final tagged main CI and Publish workflow |
| full pytest | ✅ PASS | Final tagged main CI and Publish workflow |
| benchmark smoke | ✅ PASS | Final tagged main CI |
| API + Web Docker | ✅ PASS | Final tagged main CI |
| Web production build | ✅ PASS | Final tagged main CI |
| Compose / MkDocs strict | ✅ PASS | Final tagged main CI |
| Dependency Review / Secret Scan | ✅ PASS | Release PR security workflows |
| CodeQL Python + JavaScript | ✅ PASS | Final tagged main checks |
| SonarQube Quality Gate | ✅ PASS | Release-candidate Quality Gate passed; 0 security hotspots |
| Wheel + sdist build | ✅ PASS | Final tagged main CI and Publish workflow |
| Clean-wheel install smoke | ✅ PASS | Final tagged main CI |
...
4. Let the Publish workflow rebuild and fresh-wheel smoke-test the exact tag, retain the wheel/sdist as a workflow artifact, generate attestations, and publish to PyPI through Trusted Publishing.
5. Treat PyPI as the canonical wheel/sdist distribution channel.
6. Attach wheel/sdist files to GitHub Release only when the Release is still mutable; an immutable-release rejection is non-fatal because the verified package is already preserved and published through the canonical channel.
