...
- Final candidate head `3f6b184eb9f62baf14a4a6fb84898932b63f384e` passed CI run `33289287375`, Dependency Review, Secret Scan, CodeQL, and the SonarQube Quality Gate. SonarQube reported **4 non-blocking new issues and 0 security hotspots**; the Quality Gate passed.

The release verifier was hardened during this process with an explicit command allowlist, and Compose declares the developer `.env` file optional (`required: false`) so validation remains read-only when that local file is absent.
...
| CodeQL Python + JavaScript | ✅ PASS | Both languages green with no new CodeQL alerts |
