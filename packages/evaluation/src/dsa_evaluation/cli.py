...
def main() -> None:
    ...
    ap.add_argument("--catalog", type=Path, default=Path("benchmarks/ds-agent-benchmark/catalog.json"))
    ...
    ap.add_argument("--task", action="append", dest="tasks", default=None, help="Filter to task id(s)")
    args = ap.parse_args()
    payload = run_benchmark(args.catalog, args.datasets, args.out, limit=args.limit, task_ids=args.tasks)
