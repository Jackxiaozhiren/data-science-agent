...
# JSON-RPC 2.0 endpoint for MCP clients (stateless)
@app.post("/mcp")
async def mcp_jsonrpc(request: Request) -> JSONResponse:
    ...
    if method == "tools/list":
        tools = list_mcp_tools()
        return JSONResponse({"jsonrpc": "2.0", "id": req_id, "result": {"tools": [t.model_dump(mode="json") for t in tools]}})

    if method == "tools/call":
        raw_name = params.get("name")
        name: str = raw_name if isinstance(raw_name, str) and raw_name else ""
        if not name:
            return JSONResponse({"jsonrpc": "2.0", "id": req_id, "error": {"code": -32602, "message": "Missing tool name"}}, status_code=200)
        arguments = params.get("arguments") or {}
        result = await call_mcp_tool(name, arguments if isinstance(arguments, dict) else {})
        if result.get("isError"):
            return JSONResponse({"jsonrpc": "2.0", "id": req_id, "error": {"code": -32602, "message": result.get("error")}}, status_code=200)
        ...
        return JSONResponse(
            {
                ...
                "result": {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "dsa-mcp", "version": "0.1.0"},
                },
            }
        )

    return JSONResponse({"jsonrpc": "2.0", "id": req_id, "error": {"code": -32601, "message": f"Method not found: {method}"}}, status_code=200)
...
async def stdio_main() -> None:
    ...
    while True:
        ...
        if method == "tools/list":
            tools = list_mcp_tools()
            resp = {"jsonrpc": "2.0", "id": req_id, "result": {"tools": [t.model_dump(mode="json") for t in tools]}}
        elif method == "tools/call":
            raw_name2 = params.get("name")
            name2: str = raw_name2 if isinstance(raw_name2, str) and raw_name2 else ""
            if not name2:
                resp = {"jsonrpc": "2.0", "id": req_id, "error": {"code": -32602, "message": "Missing tool name"}}
            else:
                arguments = params.get("arguments") or {}
                result = await call_mcp_tool(name2, arguments if isinstance(arguments, dict) else {})
                if result.get("isError"):
                    resp = {"jsonrpc": "2.0", "id": req_id, "error": {"code": -32602, "message": result.get("error")}}
                else:
                    resp = {"jsonrpc": "2.0", "id": req_id, "result": result}
        elif method == "initialize":
            resp = {"jsonrpc": "2.0", "id": req_id, "result": {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "dsa-mcp", "version": "0.1.0"}}}
        else:
