...
async function fetchRun(id: string): Promise<RunDetail | null> {
  const res = await fetch(`http://localhost:8000/api/v1/analysis/${id}`, { cache: "no-store" });
...
function StatusDot({ status }: { status: string }) {
  ...
  return <span className={`inline-block rounded px-2 py-1 text-xs ${ok ? "bg-green-100 text-green-700" : fail ? "bg-red-100 text-red-700" : "bg-zinc-100"}`}>{status}</span>;
}

export default async function TracePage({ params }: { params: Promise<{ runId: string }> }) {
  const { runId } = await params;
  const run = await fetchRun(runId);
  if (!run) return <p className="text-sm text-red-600">Run not found.</p>;
  const st = run.state;
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold">Analysis {runId.slice(0, 12)} <StatusDot status={run.status} /></h1>
        <a href={`/analysis/${runId}/report`} className="text-sm underline">Report view</a>
      </div>
      ...
      <div className="rounded border bg-white p-4">
        ...
        <ol className="mt-2 list-decimal pl-5 text-sm">
          {(st?.plan || []).map((s) => <li key={s.id}><span className="font-mono text-xs">{s.tool}</span> — {s.name}<span className="text-zinc-500"> · {s.description}</span></li>)}
        </ol>
      </div>
          ...
          <tbody>
            {(st?.tool_calls || []).map((tc) => (
              <tr key={tc.call_id} className="border-t">
                ...
                <td><span className={`rounded px-1 text-xs ${tc.status === "ok" ? "bg-green-50" : "bg-red-50"}`}>{tc.status}</span></td>
        ...
        <ul className="mt-2 space-y-2 text-sm">
          {(st?.evidence || []).map((ev) => (
            <li key={ev.id} className="rounded border p-2">
              ...
              <div>{ev.claim}</div>
              <div className="text-xs text-zinc-500">{JSON.stringify(ev.result).slice(0, 300)}</div>
            </li>
          ))}
      ...
      <div className="rounded border bg-white p-4">
        ...
        <ul className="mt-2 space-y-2 text-sm">
          {(st?.insights || []).map((ins) => (
            <li key={ins.id} className="rounded border p-2"><div>{ins.finding}</div><div className="text-xs text-zinc-500">Evidence: {ins.evidence_ids.join(", ") || "—"} {ins.limitation ? `· ${ins.limitation}` : ""}</div></li>
          ))}
        </ul>
      </div>
      ...
      {st?.report_markdown && (
        <div className="rounded border bg-white p-4">
          ...
          <a href={`/api-via?run=${runId}&format=markdown`} className="mt-2 inline-block text-sm underline">Download markdown — GET /api/v1/analysis/{runId}/report?format=markdown</a>
        </div>
      )}
    </div>
  );
